...
\lfoot{\textbf{xBGAS 0.0.4}}  % -- classification
...
\vspace{2in}
\begin{center}
\includegraphics[width=2in]{figures/rv128.pdf} % Include the logo
\end{center}
\end{figure}
...
\begin{tabular}{l r}
Date: & \today \\
Version: & 0.0.4 \\ % Date the experiment was performed
Authors: & John Leidel\\
& David Donofrio\\
...
as a \textit{greenfield} extension with respect to the core RISC-V 
...
that the ``W'' bit (bit-22) must be set to indicate \textit{wide} addressing mode for xBGAS.  

\begin{commentary}
Prior to becoming an official extension, you will also need to set the ``X'' bit (bit-23) in order 
to indicate a non-standard extension being present.  
\end{commentary}
...
registers beginning at index \textit{0}, we map a complementary extent 
register at the same index.  In this manner, xBGAS memory operations 
utilize both the base and extent registers at the target index in the process 
...
Any attempt to utilize extent registers below index \textit{10} will 
...
extent register, \textit{e21}, for the most significant 64 bits of the effective address.  Finally, we utilize the immediate 
...
The xBGAS extension adds a series of \emph{extent} registers to the list of user-visible registers.  Each extent register is an unsigned 64-bit register that contains the most significant 64-bits of a 128-bit memory address.  In this manner, normal 64-bit and 32-bit memory operations (RV64I and RV32I respectively) are effectively unchanged.  The xBGAS extent registers can only be utilized for memory operations when utilizing xBGAS load and store operations.  Further, extent registers can only be read from and written to from xBGAS Address Management Instructions (Section~\ref{sec:AddressManagementInstructions}).   
...
Note the explicit negation of specific extent registers in order to prohibit 
...
and its complementary extent registers \textit{ext1}.  The least significant 64 bits of the address
[63-0] are encoded in the base register.  The most significant 64 bits of the address [127-64] 
are encoded in the extent register.  
...
128-bot address at \textit{ext1+rs1}.  Store the results in \textit{rd+extd}.  The most 
significant 64-bits [127-64] are stored in the extent register \textit{extd}.  The least 
...
Load a 64-bit word from the address formed by adding the immediate value to the 
64-bit address at \textit{rs1}.  Store the results in the extent register \textit{extd}.  
...
are stored in the extent register at \textit{ext1} and the least significant 64-bits of the source 
...
the instruction explicitly specifies the base register as well as the extent register 
...
are encoded using the I-type format.  

\subsubsection{eaddi rd, ext1, imm}

Perform an unsigned 64-bit add operation of the value stored in the extent 
register \textit{ext1} with the included immediate value.  Store the result 
in the target base register \textit{rd}.  For this operation, the extent and base registers 
do not need to share the same index.  This operation may be encoded 
using an assembly shorthand for moving values from extent registers 
to base registers.
...
target extent register \textit{extd}.  This operation may be encoded 
...
target extent register \textit{extd}.  This operation may be encoded using 
an assembly shorthand for moving values between extent registers.  
...
\hline
eld rd, imm(rs1) & rs1+ext1 & 011 & rd & 0111111\\
\hline
elw rd, imm(rs1) & rs1+ext1 & 010 & rd & 0111111\\
\hline
elh rd, imm(rs1) & rs1+ext1 & 001 & rd & 0111111\\
\hline
elhu rd, imm(rs1) & rs1+ext1 & 101 & rd & 0111111\\
\hline
elb rd, imm(rs1) & rs1+ext1 & 000 & rd & 0111111\\
\hline
elbu rd, imm(rs1) & rs1+ext1 & 100 & rd & 0111111\\
\hline
\end{tabular}
\end{center}
\end{table}
...
\hline
esd rs1, imm(rs2) & rs1 & rs2+ext2 & 011 & 0111110\\
\hline
esw rs1, imm(rs2) & rs1 & rs2+ext2 & 010 & 0111110\\
\hline
esh rs1, imm(rs2) & rs1 & rs2+ext2 & 001 & 0111110\\\hline
esb rs1, imm(rs2) & rs1 & rs2+ext2 & 000 & 0111110\\
\hline
\end{tabular}
\end{center}
\end{table}
...
Mnemonic & base & funct3 & dest & opcode \\ \hline
\hline
elq rd, imm(rs1) & rs1+ext1 & 000 & rd & 1111111\\
\hline
ele extd, imm(rs1) & rs1+ext1 & 001 & rd & 1111111\\
\hline
\end{tabular}
\end{center}
\end{table}
...
Mnemonic & src & base & funct3 & opcode \\ \hline
\hline
esq rs1, imm(rs2) & rs1 & rs2+ext2 & 100 & 1111111\\
\hline
ese ext1, imm(rs2) & ext1 & rs2+ext2 & 101 & 1111111\\
\hline
\end{tabular}
\end{center}
\end{table}
...
Mnemonic & base & funct3 & dest & opcode \\ \hline
\hline
eaddi rd, ext1, imm & ext1 & 001 & rd & 1111111\\
\hline
eaddie extd, rs1, imm & rs1 &100 & extd & 1111111\\
\hline
eaddix extd, ext1, imm & extd & 101 & ext1 & 1111111\\
\hline
\end{tabular}
\end{center}
\end{table}
