...
\pagestyle{fancy}
\lhead{}
\chead{\textbf{SV-xBGAS Architecture Extension Specification}}  % -- classification
\rhead{}
\cfoot{} %-- format: TR YYYY-RRR-V.V; y = year; r = report; v = version
\lfoot{\textbf{SV-xBGAS 2.0.0}}  % -- classification
...
\title{\textbf{RISC-V Extended Addressing\\Architecture Extension Specification\\Codenamed: SV-xBGAS}} % Title
...
\subsection{SV-xBGAS Overview}

The goal of the \gls{xBGAS} SV extension described herein is to provide \textit{extended} 
...
instructions without utilizing the SV-xBGAS extensions without issue.  Further, RV32I and RV64I 
applications that are binary compatible should execute unencumbered on a system that 
complies with the SV-xBGAS extensions described herein.

\subsection{SV-xBGAS Extension ISA Requirements}

The SV-xBGAS instruction set extension described herein is classified 
...
formats and encodings.  However, the SV-xBGAS extension does instantiate 
additional machine state in terms of user-visible and supervisor registers.

The SV-xBGAS specification functions an extension to the existing RV64I~\cite{RVSpec} 
base instruction set specification (and all inherited instructions therein).  In this 
manner, the SV-xBGAS extension is truly an extension, rather than a full expansion 
of the register \textit{XLEN} and the associated ALU.

\subsection{SV-xBGAS Implementation Requirements}

The goal of this specification is to outline the required encoding, register extensions 
and associated supervisor level functionality required to implement the SV-xBGAS 
...
core pipeline model.  It is up to the implementor to define how the SV-xBGAS instructions are 
implemented.

\item \textbf{Virtualization}: This specification makes no mention of implementing a virtualization 
layer on top of SV-xBGAS-style addressing.  Do not expect that SV-xBGAS addressing modes will 
...
required to enforce enclaves on a system with SV-xBGAS extensions is beyond the scope 
of this document.

\end{itemize}

\subsection{SV-xBGAS Supervisor Specification Requirements}
...
In the case of SV-xBGAS, this value is set to represent a 64-bit base ISA (\textit{MXL=2}).  We do 
not override this value as the SV-xBGAS implementation described herein does not 
...
the ``I'' bit (bit-8) must be set for all implementations that support SV-xBGAS.  Further, we also dictate 
that the ``X'' bit (bit-23) must be set to indicate a non-standard extension.

\begin{commentary}
Given that SV-xBGAS is currently a non-standard extension, the ``X'' bit must be set.  However, 
...
formatted as per Figure~\ref{scausereg}.  The SV-xBGAS specification does not add any specific 
Exception Code values to be placed in the \wlrl\ field.  However, much in the manner of 
normal load and store operations from RV32I and RV64I, the SV-xBGAS extension has the ability 
...
\end{tabular}
\end{center}
\caption{SV-xBGAS Supervisor cause register ({\tt scause}) values after trap.}
\label{scauses}
\end{table*}
...
%----------------------------------------------------------------------------------------
\clearpage
\section{SV-xBGAS Machine Organization}

\subsection{SV-xBGAS Extended Addressing Logic}

At the core of the SV-xBGAS implementation is the ability to utilize existing RV64I 
binary blobs without issue.  In this manner, the SV-xBGAS extension makes use 
...
\caption{SV-xBGAS Effective Address Calculation}
...
value in the instruction payload for any potential offset calculations.  The contents of the \textit{e21} 
register are interpreted as the standard SV128 object ID.  This is shown in Figure~\ref{fig:objaddr}.  

\begin{figure}[h!]
\begin{center}
\includegraphics[width=3in]{figures/objaddr.pdf}
\caption{SV-xBGAS Object Addressing}
\label{fig:objaddr}
\end{center}
\end{figure}

\subsection{SV-xBGAS Address Formation}

\begin{table*}[h!]
\begin{center}
\begin{tabular}{|c|c|c|c|}
\hline
\textbf{Object Type} & \textbf{Function} & \textbf{RV32} & \textbf{RV64} \\
\hline
0-3 & Kernel Object & Yes & Yes\\
\hline
4 & Local Object & Yes & Yes\\
\hline
5 & PGAS Object & Yes & Yes\\
\hline
6-13 & Reserved & N/A & N/A\\
\hline
14 & RV64 & No & Yes\\
\hline
15 & 60 Bit Object ID & No & Yes\\

\hline
\end{tabular}
\end{center}
\caption{SV-xBGAS Object Identifiers}
\label{tab:objectids}
\end{table*}

\subsection{SV-xBGAS Register State}
...
\newpage
\subsubsection{SV-XBGAS Register Indexing}
...
\hline
Register & SV-xBGAS Mapping & Index & Description & ABI Name\\ \hline
\hline
...
%----------------------------------------------------------------------------------------
\newpage
\section{SV-xBGAS Instruction Set Extension}
...
There are currently no supervisor instructions defined in the SV-xBGAS extension.  

%----------------------------------------------------------------------------------------
%	SECTION 4
%----------------------------------------------------------------------------------------
\newpage
\section{SV-xBGAS Instruction Set Listings}

\subsection{SV-xBGAS Load/Store Instructions}
...
\newpage
\subsection{SV-xBGAS Address Management Instructions}
